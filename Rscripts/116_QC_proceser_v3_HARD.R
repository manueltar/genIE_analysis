...
Graph_HARD = function(option_list)
{
  opt_in = option_list
  opt <<- option_list
  ...
  indx.dep.DEF<-unique(c(indx.dep3,indx.dep4))
  ...
  if(dim(mismatch_CONDENSED_subset)[1] >0)
  {
    ...
    Gather_decomposed_cigar_HARD<-Gather_decomposed_cigar_subset#Gather_decomposed_cigar[which(Gather_decomposed_cigar$S > 0),]
    
    cat("Gather_decomposed_cigar_HARD\n")
    cat(str(Gather_decomposed_cigar_HARD))
    cat("\n")
    
    
    #### Add up the CIGARS by Sample_Name and BIN_order_factor----
    
    Gather_decomposed_cigar_HARD.dt <- data.table(Gather_decomposed_cigar_HARD) 
    
    DT.reads<-Gather_decomposed_cigar_HARD.dt[ , .(Totalcount = sum(reads)), 
                                               by = .(Sample_Name,BIN_order_factor,replica_order_factor)]
    
    #### Add up the reads by Sample_Name TOTAL reads that belonng to HARD in every replica ----
    
    DT.reads2<-Gather_decomposed_cigar_HARD.dt[ , .(Totalcount = sum(reads)), 
    ...
    #### merge with total HARD reads and with total Sample_Name reads  ----
    ...
    DEF_HARD$Perc<-round(100*DEF_HARD$Totalcount/DEF_HARD$TOTAL_reads,2)
    ...
    check<-DEF_HARD[is.na(DEF_HARD$Totalcount),]
    ...
    ##### LOOP TO GET Get the distribution of the size HARDs ----
    ...
    CIGARS_HARD<-unique(as.character(Gather_decomposed_cigar_HARD$CIGAR))
    
    cat("CIGARS_HARD\n")
    cat(sprintf(as.character(CIGARS_HARD)))
    cat("\n")
    
    list_S_DEF<-list()
    
    
    for(k in 1:length(CIGARS_HARD))
    {
      CIGAR_sel<-CIGARS_HARD[k]
      
      cat("------------------------------------------------>CIGAR_sel\t")
      cat(sprintf(as.character(CIGAR_sel)))
      cat("\n")
      
      Gather_decomposed_cigar_HARD_CIGAR<-Gather_decomposed_cigar_HARD[which(Gather_decomposed_cigar_HARD$CIGAR == CIGAR_sel),]
      
      colnames(Gather_decomposed_cigar_HARD_CIGAR)[which(colnames(Gather_decomposed_cigar_HARD_CIGAR) == "Sample_Name")]<-"sample"
      
      # cat("Gather_decomposed_cigar_HARD_CIGAR_POST\n")
      # cat(str(Gather_decomposed_cigar_HARD_CIGAR))
      # cat("\n")
      
      Gather_decomposed_cigar_HARD_CIGAR.dt<-data.table(Gather_decomposed_cigar_HARD_CIGAR)
      collapse.reads<-Gather_decomposed_cigar_HARD_CIGAR.dt[ , .(Totalcount = sum(reads)), 
                                                             by = .(name,H,replica_order_factor,
                                                                    sample)]
      ...
      for(l in 1:length(collapse.reads$sample))
      {
        ...
        Gather_decomposed_cigar_HARD_CIGAR_sample<-Gather_decomposed_cigar_HARD_CIGAR[which(Gather_decomposed_cigar_HARD_CIGAR$sample == sample_sel),]
        
        # cat("Gather_decomposed_cigar_HARD_CIGAR_sample_POST\n")
        # cat(str(Gather_decomposed_cigar_HARD_CIGAR_sample))
        # cat("\n")
    ...
    colnames(TABLE_CIGAR_SIZES_DEF)[which(colnames(TABLE_CIGAR_SIZES_DEF) == "H")]<-"HARD_size"
    ...
    breaks.HARD<-seq(0,100, by=10)
    labels.HARD<-as.character(breaks.HARD)
    ...
    for(i in 1:length(VARS))
    {
      ...
      if(dim(COUNT_matrix_sel)[1]>0)
      {
        ...
        for(k in 1:length(HGNC.vector))
        {
          ...
          DEF_HARD_sel<-DEF_HARD[which(DEF_HARD$sample%in%COUNT_matrix_sel_HGNC_sel$sample),]
          
          cat("DEF_HARD_sel\n")
          cat(str(DEF_HARD_sel))
          cat("\n")
          
          if(dim(DEF_HARD_sel)[1] > 0)
          {
            ...
            g.HARD<-DEF_HARD_sel %>%
              ...
              scale_y_continuous(name ="Percentage of TOTAL READS", breaks =breaks.HARD, 
                                 labels=labels.HARD, 
                                 limits = c(breaks.HARD[1],breaks.HARD[length(breaks.HARD)]))+
            ...
            #ggtitle(paste("Reads HARD",VAR_sel, HGNC_sel, sep= " ")) +
            
            # myplots_HARD[[VAR_sel]][[HGNC_sel]]<-g.HARD
            # 
            # setwd(out)
            # 
          ...
          if(dim(TABLE_CIGAR_SIZES_DEF_sel)[1] > 0)
          {
            ...
            max.log.HARD_size<-max(log10(TABLE_CIGAR_SIZES_DEF_sel$HARD_size+0.001))
            
            cat("max.log.HARD_size\n")
            cat(sprintf(as.character(max.log.HARD_size)))
            cat("\n")
            
            min.log.HARD_size<-min(log10(TABLE_CIGAR_SIZES_DEF_sel$HARD_size+0.001))
            
            cat("min.log.HARD_size\n")
            cat(sprintf(as.character(min.log.HARD_size)))
            cat("\n")
            
            breaks.HARD_size.log<-sort(unique(c(log10(1),min.log.HARD_size,seq(min.log.HARD_size,max.log.HARD_size, by=0.5),max.log.HARD_size+0.01)), decreasing = F)
            
            cat("breaks.HARD_size.log\n")
            cat(sprintf(as.character(breaks.HARD_size.log)))
            cat("\n")
            
            labels.HARD_size<-as.character(round(10^breaks.HARD_size.log,1))
            
            cat("labels.HARD_size\n")
            cat(sprintf(as.character(labels.HARD_size)))
            cat("\n")
            ...
            g.HARD.sizes<-ggplot(TABLE_CIGAR_SIZES_DEF_sel, aes(x=log10(HARD_size+0.001), 
              ...
              theme(plot.title = element_text(size=11)) +
              theme(plot.title=element_text(size=11))+
              scale_x_continuous(name="HARD size", 
                                 breaks=breaks.HARD_size.log,
                                 labels=labels.HARD_size, 
                                 limits=c(breaks.HARD_size.log[1],breaks.HARD_size.log[length(breaks.HARD_size.log)]))+
            ...
            #ggtitle(paste("Reads HARD sizes",VAR_sel, HGNC_sel, sep= " ")) +
            
            
            title <- ggdraw() + draw_label(paste("HARD",VAR_sel,HGNC_sel,sep=" "))
            
            graph_HARD_DEF<-plot_grid(title,g.HARD, g.HARD.sizes,
                                      nrow = 3,
                                      align = "v",
            ...
            myplots_HARD[[VAR_sel]][[HGNC_sel]]<-graph_HARD_DEF
            
            
            setwd(out)
            
            pdf(file=paste("test","_",VAR_sel,"_",HGNC_sel,".pdf",sep=''))
            print(graph_HARD_DEF)
            dev.off()
    ...
    saveRDS(myplots_HARD, file = filename)
