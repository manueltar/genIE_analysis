...
Graph_DEL = function(option_list)
{
  opt_in = option_list
  opt <<- option_list
  ...
  indx.dep.DEF<-unique(c(indx.dep5,indx.dep6))
  ...
  if(dim(mismatch_CONDENSED_subset)[1] >0)
  {
    ...
    Gather_decomposed_cigar_DEL<-Gather_decomposed_cigar_subset#Gather_decomposed_cigar[which(Gather_decomposed_cigar$S > 0),]
    
    cat("Gather_decomposed_cigar_DEL\n")
    cat(str(Gather_decomposed_cigar_DEL))
    cat("\n")
    
    
    #### Add up the CIGARS by Sample_Name and BIN_order_factor----
    
    Gather_decomposed_cigar_DEL.dt <- data.table(Gather_decomposed_cigar_DEL) 
    
    DT.reads<-Gather_decomposed_cigar_DEL.dt[ , .(Totalcount = sum(reads)), 
                                              by = .(Sample_Name,BIN_order_factor,replica_order_factor)]
    
    #### Add up the reads by Sample_Name TOTAL reads that belonng to DEL in every replica ----
    
    DT.reads2<-Gather_decomposed_cigar_DEL.dt[ , .(Totalcount = sum(reads)), 
    ...
    #### merge with total DEL reads and with total Sample_Name reads  ----
    ...
    DEF_DEL$Perc<-round(100*DEF_DEL$Totalcount/DEF_DEL$TOTAL_reads,2)
    ...
    check<-DEF_DEL[is.na(DEF_DEL$Totalcount),]
    ...
    ##### LOOP TO GET Get the distribution of the size DELs ----
    ...
    CIGARS_DEL<-unique(as.character(Gather_decomposed_cigar_DEL$CIGAR))
    
    cat("CIGARS_DEL\n")
    cat(sprintf(as.character(CIGARS_DEL)))
    cat("\n")
    
    list_S_DEF<-list()
    
    
    for(k in 1:length(CIGARS_DEL))
    {
      CIGAR_sel<-CIGARS_DEL[k]
      
      cat("------------------------------------------------>CIGAR_sel\t")
      cat(sprintf(as.character(CIGAR_sel)))
      cat("\n")
      
      Gather_decomposed_cigar_DEL_CIGAR<-Gather_decomposed_cigar_DEL[which(Gather_decomposed_cigar_DEL$CIGAR == CIGAR_sel),]
      
      colnames(Gather_decomposed_cigar_DEL_CIGAR)[which(colnames(Gather_decomposed_cigar_DEL_CIGAR) == "Sample_Name")]<-"sample"
      
      # cat("Gather_decomposed_cigar_DEL_CIGAR_POST\n")
      # cat(str(Gather_decomposed_cigar_DEL_CIGAR))
      # cat("\n")
      
      Gather_decomposed_cigar_DEL_CIGAR.dt<-data.table(Gather_decomposed_cigar_DEL_CIGAR)
      collapse.reads<-Gather_decomposed_cigar_DEL_CIGAR.dt[ , .(Totalcount = sum(reads)), 
                                                            by = .(name,D,replica_order_factor,
                                                                   sample)]
      ...
      for(l in 1:length(collapse.reads$sample))
      {
        ...
        Gather_decomposed_cigar_DEL_CIGAR_sample<-Gather_decomposed_cigar_DEL_CIGAR[which(Gather_decomposed_cigar_DEL_CIGAR$sample == sample_sel),]
        
        # cat("Gather_decomposed_cigar_DEL_CIGAR_sample_POST\n")
        # cat(str(Gather_decomposed_cigar_DEL_CIGAR_sample))
        # cat("\n")
    ...
    colnames(TABLE_CIGAR_SIZES_DEF)[which(colnames(TABLE_CIGAR_SIZES_DEF) == "D")]<-"DEL_size"
    ...
    breaks.DEL<-seq(0,100, by=10)
    labels.DEL<-as.character(breaks.DEL)
    ...
    for(i in 1:length(VARS))
    {
      ...
      if(dim(COUNT_matrix_sel)[1]>0)
      {
        ...
        for(k in 1:length(HGNC.vector))
        {
          ...
          DEF_DEL_sel<-DEF_DEL[which(DEF_DEL$sample%in%COUNT_matrix_sel_HGNC_sel$sample),]
          
          cat("DEF_DEL_sel\n")
          cat(str(DEF_DEL_sel))
          cat("\n")
          
          if(dim(DEF_DEL_sel)[1] > 0)
          {
            ...
            g.DEL<-DEF_DEL_sel %>%
              ...
              scale_y_continuous(name ="Percentage of TOTAL READS", breaks =breaks.DEL, 
                                 labels=labels.DEL, 
                                 limits = c(breaks.DEL[1],breaks.DEL[length(breaks.DEL)]))+
            ...
            #ggtitle(paste("Reads DEL",VAR_sel, HGNC_sel, sep= " ")) +
            
            # myplots_DEL[[VAR_sel]][[HGNC_sel]]<-g.DEL
            # 
            # setwd(out)
            # 
          ...
          if(dim(TABLE_CIGAR_SIZES_DEF_sel)[1] > 0)
          {
            ...
            max.log.DEL_size<-max(log10(TABLE_CIGAR_SIZES_DEF_sel$DEL_size+0.001))
            
            cat("max.log.DEL_size\n")
            cat(sprintf(as.character(max.log.DEL_size)))
            cat("\n")
            
            min.log.DEL_size<-min(log10(TABLE_CIGAR_SIZES_DEF_sel$DEL_size+0.001))
            
            cat("min.log.DEL_size\n")
            cat(sprintf(as.character(min.log.DEL_size)))
            cat("\n")
            
            breaks.DEL_size.log<-sort(unique(c(log10(1),min.log.DEL_size,seq(min.log.DEL_size,max.log.DEL_size, by=0.5),max.log.DEL_size+0.01)), decreasing = F)
            
            cat("breaks.DEL_size.log\n")
            cat(sprintf(as.character(breaks.DEL_size.log)))
            cat("\n")
            
            labels.DEL_size<-as.character(round(10^breaks.DEL_size.log,1))
            
            cat("labels.DEL_size\n")
            cat(sprintf(as.character(labels.DEL_size)))
            cat("\n")
            ...
            g.DEL.sizes<-ggplot(TABLE_CIGAR_SIZES_DEF_sel, aes(x=log10(DEL_size+0.001), 
              ...
              theme(plot.title = element_text(size=11)) +
              theme(plot.title=element_text(size=11))+
              scale_x_continuous(name="DEL size", 
                                 breaks=breaks.DEL_size.log,
                                 labels=labels.DEL_size, 
                                 limits=c(breaks.DEL_size.log[1],breaks.DEL_size.log[length(breaks.DEL_size.log)]))+
            ...
            #ggtitle(paste("Reads DEL sizes",VAR_sel, HGNC_sel, sep= " ")) +
            
            
            title <- ggdraw() + draw_label(paste("DEL",VAR_sel,HGNC_sel,sep=" "))
            
            graph_DEL_DEF<-plot_grid(title,g.DEL, g.DEL.sizes,
                                     nrow = 3,
                                     align = "v",
            ...
            myplots_DEL[[VAR_sel]][[HGNC_sel]]<-graph_DEL_DEF
    ...
    saveRDS(myplots_DEL, file = filename)
  }# mismatch >0
