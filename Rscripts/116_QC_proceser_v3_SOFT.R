...
Graph_SOFT = function(option_list)
{
  opt_in = option_list
  opt <<- option_list
  ...
  indx.dep.DEF<-unique(c(indx.dep,indx.dep2))
  ...
  if(dim(mismatch_CONDENSED_subset)[1] >0)
  {
    ...
    Gather_decomposed_cigar_SOFT<-Gather_decomposed_cigar_subset#Gather_decomposed_cigar[which(Gather_decomposed_cigar$S > 0),]
    
    cat("Gather_decomposed_cigar_SOFT\n")
    cat(str(Gather_decomposed_cigar_SOFT))
    cat("\n")
    
    
    #### Add up the CIGARS by Sample_Name and BIN_order_factor----
    
    Gather_decomposed_cigar_SOFT.dt <- data.table(Gather_decomposed_cigar_SOFT) 
    
    DT.reads<-Gather_decomposed_cigar_SOFT.dt[ , .(Totalcount = sum(reads)), 
                                               by = .(Sample_Name,BIN_order_factor,replica_order_factor)]
    
    #### Add up the reads by Sample_Name TOTAL reads that belonng to SOFT in every replica ----
    
    DT.reads2<-Gather_decomposed_cigar_SOFT.dt[ , .(Totalcount = sum(reads)), 
    ...
    #### merge with total SOFT reads and with total Sample_Name reads  ----
    ...
    DEF_SOFT$Perc<-round(100*DEF_SOFT$Totalcount/DEF_SOFT$TOTAL_reads,2)
    ...
    check<-DEF_SOFT[is.na(DEF_SOFT$Totalcount),]
    ...
    ##### LOOP TO GET Get the distribution of the size SOFTs ----
    ...
    CIGARS_SOFT<-unique(as.character(Gather_decomposed_cigar_SOFT$CIGAR))
    
    cat("CIGARS_SOFT\n")
    cat(sprintf(as.character(CIGARS_SOFT)))
    cat("\n")
    
    list_S_DEF<-list()
    
    
    for(k in 1:length(CIGARS_SOFT))
    {
      CIGAR_sel<-CIGARS_SOFT[k]
      
      cat("------------------------------------------------>CIGAR_sel\t")
      cat(sprintf(as.character(CIGAR_sel)))
      cat("\n")
      
      Gather_decomposed_cigar_SOFT_CIGAR<-Gather_decomposed_cigar_SOFT[which(Gather_decomposed_cigar_SOFT$CIGAR == CIGAR_sel),]
      
      colnames(Gather_decomposed_cigar_SOFT_CIGAR)[which(colnames(Gather_decomposed_cigar_SOFT_CIGAR) == "Sample_Name")]<-"sample"
      
      # cat("Gather_decomposed_cigar_SOFT_CIGAR_POST\n")
      # cat(str(Gather_decomposed_cigar_SOFT_CIGAR))
      # cat("\n")
      
      Gather_decomposed_cigar_SOFT_CIGAR.dt<-data.table(Gather_decomposed_cigar_SOFT_CIGAR)
      collapse.reads<-Gather_decomposed_cigar_SOFT_CIGAR.dt[ , .(Totalcount = sum(reads)), 
                                                             by = .(name,S,replica_order_factor,
                                                                    sample)]
      ...
      for(l in 1:length(collapse.reads$sample))
      {
        ...
        Gather_decomposed_cigar_SOFT_CIGAR_sample<-Gather_decomposed_cigar_SOFT_CIGAR[which(Gather_decomposed_cigar_SOFT_CIGAR$sample == sample_sel),]
        
        # cat("Gather_decomposed_cigar_SOFT_CIGAR_sample_POST\n")
        # cat(str(Gather_decomposed_cigar_SOFT_CIGAR_sample))
        # cat("\n")
    ...
    colnames(TABLE_CIGAR_SIZES_DEF)[which(colnames(TABLE_CIGAR_SIZES_DEF) == "S")]<-"SOFT_size"
    ...
    breaks.SOFT<-seq(0,100, by=10)
    labels.SOFT<-as.character(breaks.SOFT)
    ...
    for(i in 1:length(VARS))
    {
      ...
      if(dim(COUNT_matrix_sel)[1]>0)
      {
        ...
        for(k in 1:length(HGNC.vector))
        {
          ...
          DEF_SOFT_sel<-DEF_SOFT[which(DEF_SOFT$sample%in%COUNT_matrix_sel_HGNC_sel$sample),]
          
          cat("DEF_SOFT_sel\n")
          cat(str(DEF_SOFT_sel))
          cat("\n")
          
          if(dim(DEF_SOFT_sel)[1] > 0)
          {
            ...
            g.SOFT<-DEF_SOFT_sel %>%
              ...
              scale_y_continuous(name ="Percentage of TOTAL READS", breaks =breaks.SOFT, 
                                 labels=labels.SOFT, 
                                 limits = c(breaks.SOFT[1],breaks.SOFT[length(breaks.SOFT)]))+
            ...
            #ggtitle(paste("Reads SOFT",VAR_sel, HGNC_sel, sep= " ")) +
            
            # myplots_SOFT[[VAR_sel]][[HGNC_sel]]<-g.SOFT
            # 
            # setwd(out)
            # 
          ...
          if(dim(TABLE_CIGAR_SIZES_DEF_sel)[1] > 0)
          {
            ...
            max.log.SOFT_size<-max(log10(TABLE_CIGAR_SIZES_DEF_sel$SOFT_size+0.001))
            
            cat("max.log.SOFT_size\n")
            cat(sprintf(as.character(max.log.SOFT_size)))
            cat("\n")
            
            min.log.SOFT_size<-min(log10(TABLE_CIGAR_SIZES_DEF_sel$SOFT_size+0.001))
            
            cat("min.log.SOFT_size\n")
            cat(sprintf(as.character(min.log.SOFT_size)))
            cat("\n")
            
            breaks.SOFT_size.log<-sort(unique(c(log10(1),min.log.SOFT_size,seq(min.log.SOFT_size,max.log.SOFT_size, by=0.5),max.log.SOFT_size+0.01)), decreasing = F)
            
            cat("breaks.SOFT_size.log\n")
            cat(sprintf(as.character(breaks.SOFT_size.log)))
            cat("\n")
            
            labels.SOFT_size<-as.character(round(10^breaks.SOFT_size.log,1))
            
            cat("labels.SOFT_size\n")
            cat(sprintf(as.character(labels.SOFT_size)))
            cat("\n")
            ...
            g.SOFT.sizes<-ggplot(TABLE_CIGAR_SIZES_DEF_sel, aes(x=log10(SOFT_size+0.001), 
              ...
              theme(plot.title = element_text(size=11)) +
              theme(plot.title=element_text(size=11))+
              scale_x_continuous(name="SOFT size", 
                                 breaks=breaks.SOFT_size.log,
                                 labels=labels.SOFT_size, 
                                 limits=c(breaks.SOFT_size.log[1],breaks.SOFT_size.log[length(breaks.SOFT_size.log)]))+
            ...
            #ggtitle(paste("Reads SOFT sizes",VAR_sel, HGNC_sel, sep= " ")) +
            
            
            title <- ggdraw() + draw_label(paste("SOFT",VAR_sel,HGNC_sel,sep=" "))
            
            graph_SOFT_DEF<-plot_grid(title,g.SOFT, g.SOFT.sizes,
                                      nrow = 3,
                                      align = "v",
            ...
            myplots_SOFT[[VAR_sel]][[HGNC_sel]]<-graph_SOFT_DEF
            
            
            setwd(out)
            
            pdf(file=paste("test","_",VAR_sel,"_",HGNC_sel,".pdf",sep=''))
            print(graph_SOFT_DEF)
            dev.off()
    ...
    saveRDS(myplots_SOFT, file = filename)
